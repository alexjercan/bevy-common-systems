...
// Build-time markdown -> HTML for the wiki pages. Rendering happens here in Node
// (the webpack config calls wikiDocPage at configure time), so there is no
// runtime markdown cost and a no-JS / SEO reader still gets the full article -
// the "content in HTML, chrome via JS" split the wiki chrome relies on.
//
// - Every fence is highlighted with highlight.js into <pre><code class="hljs ...">.
...
// The page shell for a markdown doc: the same chrome as the wiki index (header/
// footer placeholders, the manifest-driven #wiki-nav aside, the crumb/h1/
// #wiki-tags, and #wiki-seealso), with a #doc-body placeholder the partials
// plugin fills with the rendered markdown after templating - so nothing runs
// over code samples. basePath is inlined here at config time.
// opts: { description, crumbParent: { slug, title } }.
function docShell(title, basePath, opts = {}) {
  ...
  const crumb = parent
    ? `<a href="${b}wiki/">Wiki</a>
                        / <a href="${b}wiki/${escapeAttr(parent.slug)}/">${escapeAttr(parent.title)}</a>
                        / ${t}`
    : `<a href="${b}wiki/">Wiki</a>
                        / ${t}`;
  return `<!doctype html>
<html lang="en">
    <head>
        ...
        <title>${t} - bevy_common_systems</title>${desc}
...
function wikiDocPage({ slug, mdPath, title, description, crumbParent, publicPath }) {
  ...
  return new HtmlWebpackPlugin({
    ...
    basePath: publicPath,
    docBody: html,
    templateContent: docShell(pageTitle, publicPath, { description, crumbParent }),
  });
}
