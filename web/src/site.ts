...
export function initSite(): void {
  ...
  // The site root is wherever the brand link points (basePath), e.g. "" at local
  // dev or "/bevy-common-systems" on project pages. It is only "active" on an
  ...
  links.forEach((link) => {
    ...
    const active = current === target || (target !== root && current.startsWith(target + '/'));
    if (active) {
