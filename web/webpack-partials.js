...
class HtmlPartialsPlugin {
  ...
  apply(compiler) {
    compiler.hooks.compilation.tap('HtmlPartialsPlugin', (compilation) => {
      ...
      hooks.beforeEmit.tapAsync('HtmlPartialsPlugin', (data, callback) => {
        const basePath = data.plugin.options.basePath || this.options.basePath || '/';
        ...
        // Markdown doc pages (see markdown.js / wikiDocPage) carry their rendered
        // body on the plugin's `docBody` option and inject it here, after
        // templating, so no templating runs over the code samples. The function
        // form of replace() keeps `$` sequences in the body (Rust) from being
        // treated as replacement patterns.
        const docBody = data.plugin.options.docBody;
        if (docBody) {
          data.html = data.html.replace('<div id="doc-body"></div>', () => docBody);
        }
