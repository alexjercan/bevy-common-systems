...
- [x] R1.1 (verified) Behaviour is preserved: `apply_offset` / `apply_scale` /
...
- [x] R1.2 (verified, good call) Markers over an auto-applier. Auto-applying
...
- [x] R1.4 (NIT) `apply_scale` bakes the "value * 100 as percent, clamped at 0"
  ...
  in the module doc.
- [x] R1.5 (verified) `node_flash`'s end value (`color_to_vec4(target)`) is not
...
- [x] R1.6 (verified) Removing the `TileFace` marker is correct: after the
