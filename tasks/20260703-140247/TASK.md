...
- [x] Add a `ComboText` HUD element in `spawn_hud`, positioned distinctly from
      the score (e.g. top-center or below the score), initially empty.
- [x] Add an `update_combo_text` system (Update, `Playing`) that reads the
      `Combo` resource: when `count >= 2` show "Combo xN" (optionally colored /
      scaled by the combo), and when `count < 2` clear it or fade it out.
- [x] Optionally fade the text alpha with the remaining combo timer (from the
...
Added a `ComboText` HUD line (under the score, state-scoped). update_combo_text
shows "Combo xN" when count>=2 and fades its alpha with the remaining window
(timer/COMBO_WINDOW), clearing when the combo ends. Verified boot.
