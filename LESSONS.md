...
- `record-numbers-from-a-rerun` (x1): every count that lands in a record comes from re-running its command at write time, with the UNIT named - a file measured as 33 comment lines and 15 comment blocks is one file and two numbers, and writing either from memory of the reading pass mixes them. Cheapest guard: paste the per-file breakdown, so the total is reconstructible instead of asserted. 20260731-172224.
- `state-what-the-checker-cannot-see` (x2): when a pass takes a route its own project checker is blind to (test intent promoted to `///` inside `#[cfg(test)]`, which rustdoc never renders and the tag checker exempts), record the decision in the same commit and draw the line in the convention doc - an unstated legitimate route is indistinguishable from gaming the check. Then check your OWN diff against that line: on its second outing the same author put a value guard behind the `///`, which is the misuse the rule was written to stop. Authoring a convention does not confer compliance with it. 20260731-172224, 20260731-172232.
