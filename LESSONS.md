...
- `state-what-the-checker-cannot-see` (x1): when a pass takes a route its own project checker is blind to (test intent promoted to `///` inside `#[cfg(test)]`, which rustdoc never renders and the tag checker exempts), record the decision in the same commit and draw the line in the convention doc - an unstated legitimate route is indistinguishable from gaming the check, and the next task re-decides it. 20260731-172224.
