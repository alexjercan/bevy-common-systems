...
- `fmt-relocates-eol-comments` (x1): an end-of-line comment is exempt from the comment-tag rule only where rustfmt will not reflow the line - after `{`, or inside a multi-line call, fmt moves it onto its own line and it becomes an untagged BLOCK. Run fmt BEFORE the convention checker; to label a magic number inside a construct, extract a named helper instead. 20260731-172223.
