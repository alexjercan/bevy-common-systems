...
- `probe-a-new-checker-both-ways` (x3): probe a checker or record-bound grep on
  ...
  proofs format gains a "positive control" field next to each `cmd:`) > skill
  prose (`plan/proofs.md`: a `cmd:` proof is not accepted until it has been shown
  to catch a case AND to skip one).
