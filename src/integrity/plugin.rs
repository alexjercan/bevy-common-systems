...
//!
//! - collisions deal damage: a fast impact deals impulse/energy damage
//!   (`on_impact_collision_deal_damage`), and a [`blast`](super::blast) sensor deals radial
//!   falloff damage (`on_blast_collision_deal_damage`); both funnel through
...
/// Plugin wiring the destruction pipeline. Requires [`HealthPlugin`](crate::health::HealthPlugin)
...
mod tests {
    ...
    fn a_disabled_root_is_destroyed_whole() {
        // The whole body dies when its root is disabled (structure at zero health).
        let mut app = integrity_core_app();
...
mod physics_tests {
    ...
    fn a_near_stationary_contact_applies_no_impact_damage() {
        ...
        let mut app = integrity_physics_app();
        let (b1, c1) = spawn_body(&mut app, Vec3::new(-100.0, 0.0, 0.0));
        let (b2, _c2) = spawn_body(&mut app, Vec3::new(100.0, 0.0, 0.0));
        settle(&mut app);
    ...
    fn a_blast_reaches_a_target_that_has_no_collision_events() {
        // Regression for the ordering bug: the blast must not depend on the target having
        // `CollisionEventsEnabled`. Here the target opts out (it had no `Health` when its
        // `ColliderOf` was added, so `on_collider_of_spawn_...` skipped it), yet the blast -
        // which owns its events - still reaches it. Before the fix the only event raised was
        // the target's, which this target never raises, so no damage landed.
        let mut app = integrity_physics_app();
        let body = app
            .world_mut()
    ...
    fn a_blast_deals_damage_only_once_when_the_target_also_has_events() {
        // When the target has its own events (a normal health-bearing node), avian raises both
        // orderings of the pair. The observer acts only on the blast-as-self ordering, so the
        // target is damaged exactly once - 60, not 120.
        let mut app = integrity_physics_app();
        let (_body, target_collider) = spawn_body(&mut app, Vec3::ZERO);
        settle(&mut app);
        assert!(
