...
- `EventKind` derive: the default `Info` is `()` (no payload) and the default name is the lowercased struct name, so an attribute-less `#[derive(EventKind)]` compiles. Pass `#[event_info(...)]` when the event carries a payload, as `03_modding` does. The old warning that the default path did not resolve (`tasks/20260703-095509`) described a defect fixed since; `attribute_less_derive_defaults_to_no_payload` in `modding::events` now guards the default (`tasks/20260731-172232`).
